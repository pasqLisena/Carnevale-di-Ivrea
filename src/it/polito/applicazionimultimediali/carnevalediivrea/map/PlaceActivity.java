...
public class PlaceActivity extends Activity {
	...
	private void renderView() {
		...
		if (place.getTeamsList() != null)
			for (Team t : place.getTeamsList()) {
				...
				int idTeamButton = getResources().getIdentifier(
		...
		if (place.hasMinigame()) {
			...
			int mg_mask = getResources().getIdentifier(place.getMinigameMask(),
					"drawable", getPackageName());
			if (mg_mask != 0)
				findViewById(R.id.ButtonMiniGame)
						.setBackgroundResource(mg_mask);
		}
	...
	public void goToBattle(View view) {
		if (GlobalRes.getCurrentPlayer().getOranges() >= 5) {
			oppTeamName = place.getTeamsList()
					.get(Integer.parseInt((String) view.getTag())).getName();
			...
			if (GlobalRes.getCurrentPlayer().getOranges() == 0) {
				Toast.makeText(getApplicationContext(),
						R.string.messZeroArance, Toast.LENGTH_LONG).show();
			} else {
				Toast.makeText(getApplicationContext(),
						R.string.messPocheArance, Toast.LENGTH_LONG).show();
			}
		}
	}
